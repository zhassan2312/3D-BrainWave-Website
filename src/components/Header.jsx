    ...
    lg:backdrop-blur-sm ${openNavigation?'bg-n-8': 'bg-n-8/90 backdrop-blur-sm'}`}>
        <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
            ...
            <nav className={`fixed top-[5rem] left-0 right-0 
                ...
                lg:lg-transparent ${openNavigation?'flex':'hidden'}`}>
                        ...
                        navigation.map((items)=>{
                            return(
                                ...
                                className={`block relative font-code text-2xl uppercase 
                                    text-n-1 transition colors 
                                    hover:text-color-1 ${items.onlyMobile ? 'lg:hidden':''} 
                                    px-6 py-6 md:py-8 lg:-mr-0.25 lg:text-xs lg:font-semibold 
                                    ${items.url ===location.hash ? 'z-2 lg:text-n-1':'lg:text-n-1/50'}`} 
                                key={items.id} 
                                href={items.url}
                                >
                                    {items.title}
